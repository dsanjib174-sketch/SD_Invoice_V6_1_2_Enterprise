...
def clients():
    ...
    if request.method == "POST":
        ...
        existing_client = next((c for c in clients_data if c.get("email") == email), None)
        if existing_client:
        ...
        if not expire_date:
            expire_date = (active_dt + timedelta(days=365)).strftime("%Y-%m-%d")
...
def plans():
    ...
    if request.method == "POST":
        ...
        plans_data = [p for p in plans_data if p.get("id") != plan_record["id"]]
...
def updates():
    ...
    return render_template("superadmin/updates.html", updates=updates_data)
...
def client_data():
    clients_data = load_json(CLIENTS_FILE)
    return render_template("superadmin/client_data.html", clients=clients_data)
...
def subscription():
    ...
    if session.get("login_type") == "superadmin":
        ...
        subscriptions = [c for c in clients_data if c.get("email") == current_email]

    return render_template("superadmin/subscription.html", subscriptions=subscriptions)
...
def client_updates():
    updates_data = load_json(UPDATES_FILE)
    published = [u for u in updates_data if u.get("status") == "Published"]
    return render_template("client/updates.html", updates=published)
